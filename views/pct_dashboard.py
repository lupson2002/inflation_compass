"""Percentile Channels TAA — 대시보드.

pct.db의 backtest_* 테이블만 읽어 그래프/통계/비중/상대성과/연도별 표시.
재계산 없이 DB를 직접 소비한다.
"""
...
GOOD = "#2f7d4f"
BAD = "#b0442f"
WEIGHT_COLORS = {"VTI": "#2a78d6", "IYR": "#eb6834", "LQD": "#1baf7a", "DBC": "#eda100", "SHY": "#a8a9a6"}
...
tab_chart, tab_stats, tab_rel, tab_yearly, tab_weights = st.tabs(["그래프", "통계표", "상대성과", "연도별 수익률", "비중"])

with tab_chart:
    fig1 = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.7, 0.3], vertical_spacing=0.06)
    fig1.add_trace(go.Scatter(x=bench_eq.index, y=bench_eq, name="VTI B&H", line=dict(color=SLATE, width=2)), row=1, col=1)
    fig1.add_trace(go.Scatter(x=zero_eq.index, y=zero_eq, name="Percentile Channels (0%)", line=dict(color=COPPER_DIM, width=1.4, dash="dot")), row=1, col=1)
    fig1.add_trace(go.Scatter(x=net_eq.index, y=net_eq, name=f"Percentile Channels ({cost_pct:.2f}%)", line=dict(color=COPPER, width=2)), row=1, col=1)
...
with tab_stats:
    ...
    stats_df = pd.DataFrame({
        "VTI": {k: fmt_metric(k, bench_stats[k]) for k in metric_order},
...
with tab_rel:
    ...
    fig3.update_yaxes(type="log", tickformat=".1f", ticksuffix="x", title="Strategy / VTI", row=1, col=1)
